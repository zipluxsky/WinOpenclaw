...
import { ensureOpenClawCliOnPath, getPathFromEnv } from "../infra/path-env.js";
...
function sanitizeEnv(
  ...
  const basePath = getPathFromEnv(process.env as Record<string, string | undefined>) || DEFAULT_NODE_PATH;
...
function ensureNodePathEnv(): string {
  ensureOpenClawCliOnPath({ pathEnv: getPathFromEnv() });
  const current = getPathFromEnv();
  if (current.trim()) {
    return current;
  }
