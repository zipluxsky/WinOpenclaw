...
/** Read PATH from env; on Windows the variable is often "Path" (capital P). */
export function getPathFromEnv(env: Record<string, string | undefined> = process.env): string {
...
export function ensureOpenClawCliOnPath(opts: EnsureOpenClawPathOpts = {}) {
  ...
  const existing = opts.pathEnv ?? getPathFromEnv();
